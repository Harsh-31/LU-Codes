...
public class l001
{
    ...
    {
        int l1 = searchVtx(u,v); //l1--> location1 
        graph[u].remove(l1);

        int l2 = searchVtx(v,u); //l2--> location2
        graph[v].remove(l2);
    }
    ...
    public static pair minWeightPath(int src, int dest, boolean[] vis)
    {
        if(src == dest)
        {
            return new pair(src + "", 0);
        ...
        pair myAns = new pair("", 100);
        for(Edge e : graph[src])
        {
            if(!vis[e.v])
            {
                pair recAns = minWeightPath(e.v,dest,vis);
    ...
    public static void constructGraph()
    {
        ...
        pair ans2 = minWeightPath(0,6,vis);
        System.out.println(ans2.path + " @ " + ans2.weight);
    }
